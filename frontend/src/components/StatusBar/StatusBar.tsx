...
export function StatusBar() {
  ...
  const tickerMessageParts = useMemo<TickerPart[]>(() => {
    ...
    const currentMonthHolidays = NSE_HOLIDAYS.filter(h => {
    ...
    if (currentMonthHolidays.length > 0) {
      ...
      const upcomingHolidays = NSE_HOLIDAYS.filter(h => new Date(h.dateStr) >= new Date(now.getFullYear(), now.getMonth(), 1))
