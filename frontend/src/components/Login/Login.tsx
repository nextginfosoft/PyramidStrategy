    ...
    <div className="min-h-screen bg-gray-950 flex items-center justify-center p-4 relative overflow-hidden font-sans">
      ...
      <div className="w-full max-w-md relative z-10">
        ...
        <div className="text-center mb-8 animate-fade-in">
          <div className="inline-block p-4 bg-gray-900/80 border border-gray-800 rounded-2xl shadow-xl mb-4">
          ...
          <p className="text-sm text-gray-400 mt-2">Automated Multi-User NIFTY Options Trading</p>
        </div>

        {/* Auth Container with Glassmorphism */}
        <div className="bg-gray-900/60 backdrop-blur-xl border border-gray-800 rounded-2xl shadow-2xl p-8 transition-all duration-300">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              ...
              <input
                ...
                className="w-full bg-gray-800/80 border border-gray-700/80 focus:border-orange-500/80 focus:ring-1 focus:ring-orange-500/50 rounded-xl px-4 py-3 text-sm text-white focus:outline-none transition-all duration-200"
                required
                autoFocus
              />
            </div>
            ...
            {error && (
              <div className="bg-red-900/30 border border-red-800/80 rounded-xl px-4 py-3 text-xs text-red-300 animate-shake">
                {error}
              </div>
            )}

            {success && (
              <div className="bg-emerald-900/30 border border-emerald-800/80 rounded-xl px-4 py-3 text-xs text-emerald-300">
                {success}
              </div>
            )}
