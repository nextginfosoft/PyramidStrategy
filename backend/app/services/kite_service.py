...
class KiteService:
    ...
    def stop_ticker(self):
        ...
        close() just closes this one connection and stops its retry loop,
        leaving the shared reactor running so a fresh KiteTicker can attach
        to it later (e.g. after a daily access_token refresh).
        """
        if self._ticker:
            try:
                self._ticker.close()
